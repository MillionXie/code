...
def main() -> None:
    ...
    cfg_file = load_config(args.config) if args.config is not None else {}
    ...
    if args.use_checkpoint_config and isinstance(ckpt_cfg, dict):
        ...
        raise ValueError("No config found. Provide --config or use checkpoint with embedded config.")
    ...
    logger.info(
        "Eval map done | recon_mse=%.6f recon_psnr=%.3f recon_ssim=%.4f interp_fid=%s recon_pairs=%d interp_panels=%d",
        recon_metrics["mse"],
        recon_metrics["psnr"],
        recon_metrics["ssim"],
